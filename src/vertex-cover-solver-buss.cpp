...
class Graph
{
    public:
        void load (string filePath)
        {
            ...
            cout << endl << "After removing vertex 3" << endl;
            printMatrix (removeVertex (graph, 3));
            printVector(getKernel(3).second);
        }
    ...
    private:
        ...
        vector<int> createAllCover (const int size)
        {
            vector<int> result;
        ...
        */
        ...
        pair<int, vector<int>> getKernel (int k)
        {
            vector<int> kernel = createAllCover (neighbors.size());
            vector<vector<int>> temp_graph = graph;
            vector<vector<int>> temp_neighbors = neighbors;
            ...
            {
                edgesLeft += neighbors.at (i).size();
            }

            bool reductionPossible = edgesLeft <= k2;
            ...
            while (reductionPossible)
            {
                ...
                {
                    /*
                        1. If v is a vertex of degree greater than k, remove v from the
                           graph and decrease k by one. Every vertex cover of size k
                           must contain v, since otherwise too many of its neighbors
                           would have to be picked to cover the incident edges.
                           Thus, an additional vertex cover for the original graph may
                           be formed from a cover of the reduced problem by adding v
                           back to the cover.
                    */
                    int degree = temp_neighbors.at (i).size();

                    if (degree > k)
                    {
                        kernel[i] = 0;
                        edgesLeft -= degree;
                        temp_graph = removeVertex (temp_graph, i);
                        --k;
                        k2 = k * k;
                    }

                    /*
                        2. If v is an isolated vertex, remove it. An isolated vertex
                           cannot cover any edges, so it cannot be a part of a vertex
                           cover.
                    */
                    ...
                    /*
                        3. If more than k^2 edges remain in the graph and neither of the
                           previous two rules can be applied, then the graph cannot
                           contain a vertex cover of size k. For, after eliminating all
                           vertices of degree greater than k, each remaining vertex can
                           only cover at most k edges and a set of k vertices could only
                           cover at most k^2 edges. In this case, the graph may be
                           replaced by the complete graph K_{k+1}, which also has no
                           k-vertex cover.
                     */

                    else if (edgesLeft > k2)
                    {
                        reductionPossible = false;
                        break;
                        //throw "The graph cannot contain a vertex cover of size k.";
                    }
                }
            }

            return pair<int, vector<int>> (kernel_size, kernel);
        }
