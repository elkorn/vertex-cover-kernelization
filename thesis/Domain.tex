...
\par{
  Jako \emph{klasę złożoności} określa się zbiór problemów o~podobnej
  złożoności w~pewnym kontekście. 
  Proste klasy złożoności określić można za pomocą podstawowych czynników, takich
  jak rodzaj problemu (obliczeniowy, decyzyjny, optymalizacyjny etc.), model
...
\par{
  Problemy klasy $\mathcal{FPT}$ są rozwiązywalne w~czasie $f(k)*n^{O(1)}$, gdzie $n$ stanowi rozmiar danych wejściowych, dla
  ...
  Najważeniejszą przesłanką sformułowania klasy $\mathcal{FPT}$ jest wykluczenie
...
\par{
  ...
  istnieje zbiór wierzchołków $C \in V, |C| \leq k$ pokrywający każdą krawędź tego grafu?''.
  Pokrycie wierzchołkowe stanowi zbiór wierzchołków $C
  \subseteq V; \forall_{e=(u,v) \in E}:u\in C\lor v\in C$.
...
\par{
  ...
  wykrywaniem wyścigów w~danych~\cite{O'Callahan:2003:HDD:781498.781528}.
}
\begin{theorem}
  Optymalne pokrycie wierzchołkowe grafu $G=(V,E), |V|=n, |E|=m$ może zostać 
  odnalezione w~czasie $O(2^{n}m)$.
\end{theorem}
\begin{bproof}
...
\par{
  ...
  \begin{bproof}
    ...
    Jeżeli więc istnieje krawędź łącząca dwa wierzchołki $(u, v) \in E$, z których żaden nie należy do zbioru $C$, należy dodać jeden z~wierzchołków do $C$.
    ...
    Kiedy rekurencja dociera do momentu, gdzie $k=0$ i istnieje krawędx $e=(u,v) \in E$ spełniająca własność $\neg(u \in C \lor v \in C)$, wiadomo, że rozwiązanie odnalezione w~tej gałęzi nie jest akceptowalne i należy je odrzucić.
    Bez względu na ostateczną postać rozwiązania zadanego problemu, algorytm konsekwentnie zmniejsza rozmiar parametru poszukiwanego częściowego pokrycia.
  ...
  \begin{algorithm}
    ...
    \begin{algorithmic}[1]
      \Function{C}{$G$, $k$}
        ...
        \algorithmicrequire{graf wejściowy $G=(V, E)$, największa dopuszczalna liczebność pokr. wierzch. $k$}

        \algorithmicensure{informacja czy istnieje pokr. wierzch. o liczebności $\leq k$}

        \If {$E=\emptyset$}
          \Return{true}\Comment{Odnaleziono pokr. wierzch. liczebności $\leq k$}
        \EndIf
        \If {$k=0$}
          \Return{false}\Comment{Rozwiązanie nieoptymalne}
        \EndIf
        \State $(u,v) \leftarrow e \in E$
        \State $G^\prime \gets G[V\setminus \{u\}]$\Comment{Graf zaindukowany zbiorem $V\setminus \{u\}$}
