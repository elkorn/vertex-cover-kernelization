...
\par{
  ...
  \begin{enumerate}[(1)]
    ...
    \item Struktura $\Gamma$ jest dobrą parą wierzchołków $(u, z)$ o stopniu wierzchołka $u$ wynoszącym $d(u)=4$ oraz gdzie wierzchołek $u$ posiada co najmniej trzy wierzchołki sąsiednie $v_u \in N(u)$ stopnia $d(v_u)=5$ i dodatkowo wśród wierzchołków $v_u$ istnieje co najmniej jedna krawędź.
    \item Struktura $\Gamma$ jest dobrą parą wierzchołków $(u, z)$ o stopniu wierzchołka $u$ wynoszącym $d(u)=3$ oraz gdzie wszystkie wierzchołki sąsiednie $v_u \in N(u)$ są stopnia $d(v_u)=5$ i nie współdzielą żadnych wierzchołków sąsiednich poza wierzchołkiem $u$.
  ...
  Kluczową obserwacją dotyczącą zestawienia struktur mogących znajdować się w kolejce $T$ jest zauważenie, iż powyższa lista jest wyczerpująca.
  \begin{theorem}
  ...
  \begin{bproof}
    ...
    Jeżeli graf $G$ posiada wierzchołek $v$ stopnia $d(v) \geq 7$, spełniona musi być przynajmniej jedna z własności (8), (11).
    W przeciwnym wypadku graf $G$ posiada wierzchołek $v$ stopnia $d(v) \leq 6$.
