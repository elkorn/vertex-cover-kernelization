...
\par {
  ...
  Funkcja nierozgałęziających się operacji w ramach algorytmu Chen, Kanj, Xia jest podobna do funkcji algorytmów przetwarzania wstępnego omówionych w podrozdziale~\ref{Section_preprocessing}.\\
...
\par{
  \begin{theorem}\thlabel{th_general_fold}
    ...
    \begin{enumerate}
      \item Jeżeli graf zaindukowany z grafu $G$ zbiorem $N(I)$ (w rozumieniu Definicji~\ref{def_induced_graph}) nie stanowi zbioru niezależnego, to istnieje optymalne pokrycie wierzchołkowe zawierające zbiór $N(I)$ i wykluczające zbiór $I$.
...
\par{
  ...
  \begin{bproof}
    ...
    Operacja wyznaczania pseudokorony zajmuje $O(k^{\prime}^3\sqrt{k^\prime})=O(k^3\sqrt{k})$ czasu, ponieważ należy przeprowadzić co najwyżej $O(k)$ NT--redukcji dla zaindukowanych grafów.
