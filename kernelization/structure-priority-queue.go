...
func (s *structure) computePriority(g *graph.Graph) structurePriority {
	/*
	...
	*/
	...
	switch cardinality {
	case 1:
		utility.Debug("1 vertex: %v", elements[0])
	...
	case 2:
		...
		du := g.Degree(u)
		dz := g.Degree(z)
		utility.Debug("Good pair (%v, %v), d(%v)=%v, d(%v)=%v", u, z, u, du, z, dz)
		...
		*/
		if s.q == 1 &&
			dz >= 1 && du >= dz &&
			!g.HasEdge(u, z) {
			utility.Debug("%v, %v are disjoint", u, z)
			// It's a 2-tuple.
			/*
		...
		if du == 3 || du == 4 {
			utility.Debug("%v neighbors of deg. 5", degree5NeighborsCount)
			if hasOnlyDegree5Neighbors {
				utility.Debug("has only neighbors of deg. 5", degree5NeighborsCount)
			}
...
func (self *goodPair) computePriority(g *graph.Graph) structurePriority {
...
func (self *StructurePriorityQueueProxy) Push(node *structure, g *graph.Graph) {
...
func (pq *StructurePriorityQueue) Push(x interface{}) {
	...
	utility.Debug("Push (%v) %v", item.value, item.priority)
	item.index = n
	*pq = append(*pq, item)
}
func (pq *StructurePriorityQueue) Pop() interface{} {
	...
	n := len(old)
	item := old[n-1]
	utility.Debug("Pop (%v) %v", item.value, item.priority)
